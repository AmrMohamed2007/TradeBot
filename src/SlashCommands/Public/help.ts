...
const Ping = {
    ...
    run: async (client: Client, interaction: CommandInteraction, langdata: any) => {
        ...
        const collecter = await interaction.channel.createMessageComponentCollector({ filter: u => u.user.id == interaction.user.id })
        ...
        collecter.on("collect", async col => {
            ...
            if (col.isStringSelectMenu()) {
                if (col.customId == "helpselect") {
                    ...
                    await col.reply({content:`${langdata.questions.help.find(m => m.value == value).label}\n\n${answer}`,ephemeral:true})
